...
#define CUSTOM_TILE_ROW 3   // adjustable
#define CUSTOM_TILE_COL 4   // adjustable
#define NUM_COMPUTE_WARPS (CUSTOM_TILE_ROW * CUSTOM_TILE_COL)

// DMA warps per block
#define NUM_DMA_WARPS 12    // adjustable

// Total warps in the block
#define BLOCK_WARPS (NUM_COMPUTE_WARPS + NUM_DMA_WARPS) 

// Thread block dimensions
#define BLOCK_DIM_X 32        // Fixed
...
// it loads one tile of A and one tile of B from global to shared memory by the DMA warps.
__device__ void load_global_to_shared(int k_base, int ni, int nj, int nk, int block_start_A, int block_start_B, const __half *a, const __half *b, __half* sh_A_buf, __half* sh_B_buf)
{
    // DMA warpid identification
    int dma_warp_id = (threadIdx.y - NUM_COMPUTE_WARPS);      // remaining warps after subtracting the COMPUTE_WARPS from total_warps
    int dma_tid = threadIdx.x + dma_warp_id * 32;             // local threadId respective to DMA_warps
    int dma_stride = NUM_DMA_WARPS * 32;                      // Total DMA threads

    // Load sh_A (BLOCK_TILE_M * BLOCK_TILE_K elements)
    for (int i = dma_tid; i < (BLOCK_TILE_M * BLOCK_TILE_K); i += dma_stride)
    {
        int row = i / BLOCK_TILE_K; // 0-(BLOCK_TILE_M * BLOCK_TILE_K)
        int col = i % BLOCK_TILE_K; // 0-15
        
        int gbl_r = block_start_A + row;
        int gbl_c = k_base + col;

        if (gbl_r < ni && gbl_c < nk)
            sh_A_buf[i] = a[gbl_r * nk + gbl_c]; // 1D indexing for sh_A_buf
        else
            sh_A_buf[i] = __float2half(0.0f);
    }

    // Load sh_B (BLOCK_TILE_K * BLOCK_TILE_N elements)
    for (int i = dma_tid; i < (BLOCK_TILE_K * BLOCK_TILE_N); i += dma_stride)
    {
        int row = i / BLOCK_TILE_N; // logical row 0-15
        int col = i % BLOCK_TILE_N; // logical col 0-(BLOCK_TILE_K * BLOCK_TILE_N)

        int gbl_r = k_base + row;
        int gbl_c = block_start_B + col;
        
        // Calculate 1D index in padded shared memory
        int padded_sh_idx = row * SHMEM_B_N_DIM_PADDED + col;
...
{
    //
    __shared__ __half sh_A_raw[2][BLOCK_TILE_M * BLOCK_TILE_K];
    __shared__ __half sh_B_raw[2][BLOCK_TILE_K * SHMEM_B_N_DIM_PADDED];
    ...
    int block_start_A = blockIdx.y * BLOCK_TILE_M;
    int block_start_B = blockIdx.x * BLOCK_TILE_N;
    ...
    if (is_compute_warp)
    {
        ...
        warp_row = compute_warp_id / CUSTOM_TILE_COL; // 0-3
        warp_col = compute_warp_id % CUSTOM_TILE_COL; // 0-3
        
        //
        fill_fragment(accum, 0.0f);
    }

    // k is the loop index over tile-iteration on input matrices A and B.
    int k_base = 0; // k_base used as index for DMA warps to load next tile data(next address to load by DMA)
    const int num_k_tiles = nk / BLOCK_TILE_K;

    //
    if (is_dma_warp)
    {
        load_global_to_shared(k_base, ni, nj, nk, block_start_A, block_start_B, a, b, sh_A_raw[0], sh_B_raw[0]);
    }
    __syncthreads();
    ...
    {
        int buf_idx = k_tile % 2;      // Buffer to COMPUTE from
        int next_buf_idx = 1 - buf_idx; // Buffer to LOAD into
        
        // Update k_base for the *next* tile to be loaded
        k_base = (k_tile + 1) * BLOCK_TILE_K;
        
        // DMA Warps to Load data for the *next* iteration (k+1)
        if (is_dma_warp && (k_tile < num_k_tiles - 1))
        {
            load_global_to_shared(k_base, ni, nj, nk, block_start_A, block_start_B, a, b, sh_A_raw[next_buf_idx], sh_B_raw[next_buf_idx]);
            // int endtime = clock();
            // if(threadIdx.x % 32 ==0) printf("DMA_clock(%d, %d), warp-id-%d, k_tile_%d: %d\n", blockIdx.x, blockIdx.y, warp_id, k_tile, endtime);

        }

        // Compute Warps to Process data from the *current* iteration (k)
        if (is_compute_warp)
        {
            //
            ...
            load_matrix_sync(frag_a, &sh_A_buf[warp_row * 16][0], BLOCK_TILE_K); // ld = 16

            // Load this warp's 16x16 tile from sh_B
            // Pass the PADDED leading dimension
            load_matrix_sync(frag_b, &sh_B_buf[0][warp_col * 16], SHMEM_B_N_DIM_PADDED); // ld = 72
    ...
    if (is_compute_warp)
    {
        ...
        int c_row = block_start_A + warp_row * 16;
        int c_col = block_start_B + warp_col * 16;
    ...
    DATA_TYPE POLYBENCH_2D(B,NK,NJ,nk,nj), DATA_TYPE POLYBENCH_2D(C,NI,NJ,ni,nj), DATA_TYPE POLYBENCH_2D(C_outputFromGpu,NI,NJ,ni,nj))
{
    ...
    printf("GPU Time in seconds: %f secs\n", milliseconds * 0.001); // Modified label
