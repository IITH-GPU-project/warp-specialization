...
# CONFIGURATION
# ============================================
NUM_RUNS=5   # Number of times each executable is run
OUTPUT_CSV="GEMM_tensor_results.csv"
OUTPUT_TXT="GEMM_tensor_results.txt"

# Matrix sizes to test
SIZES=(32 64 128 256 512 1024 2048 4096 8192)

CUH_FILE="gemm_tensor.cuh"

echo "Number of runs per executable = $NUM_RUNS"
echo "MatrixSize,BL_GPU,BL_CPU,BL_Mis,SH_GPU,SH_CPU,SH_Mis,WARP_GPU,WARP_CPU,WARP_Mis,WARP_SINGLE_GPU,WARP_SINGLE_CPU,WARP_SINGLE_Mis,Speedup_BL_vs_SH,Speedup_SH_vs_WARP" > $OUTPUT_CSV

# Reset TXT file
echo "===================== TENSOR GEMM RESULTS =====================" > $OUTPUT_TXT
echo "" >> $OUTPUT_TXT
...
extract_gpu() {
    local val
    val=$(echo "$1" | grep -oP "GPU Time in seconds:\s*\K[0-9.]+")
    echo ${val:-0}
}

extract_cpu() {
    local val
    val=$(echo "$1" | grep -oP "CPU Time in seconds:\s*\K[0-9.]+")
    echo ${val:-0}
}

extract_mis() {
    local val
    val=$(echo "$1" | grep -oP "Non-Matching.*:\s*\K[0-9]+")
    echo ${val:-0}
}
...
run_and_average() {
    ...
    AVG_GPU=$(awk -v a="$TOTAL_GPU" -v n="$NUM_RUNS" 'BEGIN { printf "%.6f", a/n }')
    AVG_CPU=$(awk -v a="$TOTAL_CPU" -v n="$NUM_RUNS" 'BEGIN { printf "%.6f", a/n }')
...
write_table() {
    ...
    {
        echo "==================== GEMM Tensor SUMMARY (SIZE = $SIZE) ===================="
        printf "%-35s | %-12s | %-12s | %-8s\n" "Variant" "GPU Time (s)" "CPU Time (s)" "Mismatch"
        echo "-------------------------------------------------------------------------------"
        printf "%-35s | %-12.6f | %-12.6f | %-8d\n" "Tensor Baseline" $BL_GPU $BL_CPU $BL_MIS
        printf "%-35s | %-12.6f | %-12.6f | %-8d\n" "Tensor Shared Memory" $SH_GPU $SH_CPU $SH_MIS
        printf "%-35s | %-12.6f | %-12.6f | %-8d\n" "Tensor Warp Specialization" $W_GPU $W_CPU $W_MIS
        printf "%-35s | %-12.6f | %-12.6f | %-8d\n" "Tensor Warp Single Warp" $WS_GPU $WS_CPU $WS_MIS
        echo "-------------------------------------------------------------------------------"
...
do
    echo "========================================"
    echo "Running GEMM Tensor Variants for Size = $SIZE"
    echo "========================================"

    # Update NI/NJ/NK in the gemm_tensor.cuh file
    ...
    # Run Baseline
    RES_BL=$(run_and_average "./gemm_tensor_baseline.exe")
    BL_GPU=$(echo $RES_BL | cut -d',' -f1)
    BL_CPU=$(echo $RES_BL | cut -d',' -f2)
    BL_MIS=$(echo $RES_BL | cut -d',' -f3)

    # Run Shared Memory
    RES_SH=$(run_and_average "./gemm_tensor_shared_mem.exe")
    SH_GPU=$(echo $RES_SH | cut -d',' -f1)
    SH_CPU=$(echo $RES_SH | cut -d',' -f2)
    SH_MIS=$(echo $RES_SH | cut -d',' -f3)

    # Run Warp Specialization
    RES_W=$(run_and_average "./gemm_tensor_warp_spl.exe")
    W_GPU=$(echo $RES_W | cut -d',' -f1)
    W_CPU=$(echo $RES_W | cut -d',' -f2)
    W_MIS=$(echo $RES_W | cut -d',' -f3)

    # Run Warp Single Warp
    RES_WS=$(run_and_average "./gemm_tensor_warp_spl_single_warp.exe")
    WS_GPU=$(echo $RES_WS | cut -d',' -f1)
    WS_CPU=$(echo $RES_WS | cut -d',' -f2)
    WS_MIS=$(echo $RES_WS | cut -d',' -f3)
    ...
    # Write row to CSV
    echo "${SIZE},${BL_GPU},${BL_CPU},${BL_MIS},${SH_GPU},${SH_CPU},${SH_MIS},${W_GPU},${W_CPU},${W_MIS},${WS_GPU},${WS_CPU},${WS_MIS},${BL_vs_SH},${SH_vs_WARP}" >> $OUTPUT_CSV
