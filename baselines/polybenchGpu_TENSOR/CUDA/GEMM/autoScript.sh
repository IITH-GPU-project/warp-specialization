...
OUTPUT_CSV="results_tensor_naive.csv"
...
do
    ...
    # Extract GPU time
    GPU_TIME=$(echo "$OUTPUT" | grep -A1 "GPU Time (WMMA) in seconds:" | tail -n1)

    # Extract CPU time
    CPU_TIME=$(echo "$OUTPUT" | grep -A1 "CPU Time in seconds:" | tail -n1)

    # Extract mismatches
    MISMATCH=$(echo "$OUTPUT" | grep -oP "Non-Matching CPU-GPU Outputs Beyond Error Threshold of 0.50 Percent:\s\K[0-9]+")
    MISMATCH=${MISMATCH:-0}
