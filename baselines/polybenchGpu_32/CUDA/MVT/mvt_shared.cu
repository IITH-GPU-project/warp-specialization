...
__global__ void mvt_kernel1(int n, DATA_TYPE *a, DATA_TYPE *x1, DATA_TYPE *y_1)
{
	...
	for (int t = 0; t < (_PB_N - 1) / TILE + 1; ++t){
...
__global__ void mvt_kernel2(int n, DATA_TYPE *a, DATA_TYPE *x2, DATA_TYPE *y_2)
{
	...
	for (int t = 0; t < (_PB_N - 1) / TILE2 + 1; ++t){ //Loading tile A
		 int tile_row = t * TILE2 + threadIdx.y;
		 int tile_col = blockIdx.x * blockDim.x + threadIdx.x;

		 if (tile_row < _PB_N && tile_col < _PB_N)
			...
			DATA_TYPE POLYBENCH_1D(x1_outputFromGpu, N, n), DATA_TYPE POLYBENCH_1D(x2_outputFromGpu, N, n))
{
	...
	mvt_kernel2<<<grid2,block2>>>(n, a_gpu,x2_gpu,y_2_gpu);
