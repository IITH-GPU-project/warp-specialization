...
OUTPUT_CSV="GEMM_fp32_results.csv"
OUTPUT_TXT="GEMM_fp32_results.txt"

# Matrix sizes to test
SIZES=(32 64 128 256 512 1024 2048 4096 8192)

CUH_FILE="gemm_fp32.cuh"
...
# Reset TXT file
echo "===================== FP32 GEMM RESULTS =====================" > $OUTPUT_TXT
echo "" >> $OUTPUT_TXT
...
write_table() {
    ...
    {
        echo "==================== GEMM FP32 SUMMARY (SIZE = $SIZE) ===================="
        printf "%-30s | %-12s | %-12s | %-8s\n" "Variant" "GPU Time (s)" "CPU Time (s)" "Mismatch"
        echo "--------------------------------------------------------------------------"
        printf "%-30s | %-12.6f | %-12.6f | %-8d\n" "FP32 Baseline" $BL_GPU $BL_CPU $BL_MIS
        printf "%-30s | %-12.6f | %-12.6f | %-8d\n" "FP32 Shared Memory" $SH_GPU $SH_CPU $SH_MIS
        printf "%-30s | %-12.6f | %-12.6f | %-8d\n" "FP32 Warp Specialization" $W_GPU $W_CPU $W_MIS
        echo "--------------------------------------------------------------------------"
