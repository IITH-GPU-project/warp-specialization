/**
 * mvt_ws.cu: This file is part of the PolyBench/GPU 1.0 test suite.
...
__global__ void mvt_kernel1_ws(DATA_TYPE *a, DATA_TYPE *x1, DATA_TYPE *y_1)
{
	...
	// --- FIX: Use TILE for block row dimension ---
	int row_start = blockIdx.x * TILE; // Correct: Block starts at TILE * blockIdx.x
	int rows_in_block = min(TILE, max(0, N - row_start)); // Correct: Block handles TILE rows, capped by N
	int compute_row = (is_loader) ? -1 : (lane_id + (warp_id - Loader_Warps) * 32);
    int num_tiles = (N - 1) / (TILE) + 1;
    int global_row_A = (TILE * blockIdx.x); // Correct: Global row start for A strip
    // ---------------------------------------------
    ...
    for (int t = 0; t < num_tiles; ++t) {
		...
		if (!is_loader) {
            ...
            // r iterates over the rows this block is responsible for (0 to rows_in_block-1)
