...
__global__ void mvt_kernel1(DATA_TYPE *a, DATA_TYPE *x1, DATA_TYPE *y_1)
{
    ...
    const int NUM_WARPS = COMPUTE_WARPS; // blockDim.x / Threads_Per_Warp
    ...
    // Each warp group of size Parallel_Compute is assigned a row.
    // row_warp_id runs from 0 to ROW_WARPS * Parallel_Compute - 1 (which is COMPUTE_WARPS - 1)
    
    // row_group_id determines which row this group of warps is responsible for.
    int row_group_id = warp_id / Parallel_Compute; 
    
    // warp_in_group determines the column chunk this warp works on.
    int warp_in_group = warp_id % Parallel_Compute;

	// Global row index: row_group_id * TILE + threadIdx.x is not general.
    // Use blockIdx.x to select the row block, and row_group_id to step through rows in the block.
	...
	for (int t = 0; t < (N - 1) / Tile + 1; ++t){
        ...
        // Only proceed if the row index is within N AND this warp group is assigned a row within the block.
		 if (row < N && row_group_id < ROW_WARPS)
         {
            ...
            // Warp-level reduction: Sum partial results within the warp.
            #pragma unroll
            ...
            // Atomic update: Only the first lane of the warp contributes its final sum to the global result.
            if (lane_id == 0) 
            {
