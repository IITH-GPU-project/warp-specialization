...
__global__ void mvt_kernel1_ws(DATA_TYPE *a, DATA_TYPE *x1, DATA_TYPE *y_1)
{
    ...
    // The group of PARALLEL_COMPUTE warps assigned to a single row start at:
    int row_group_start = (compute_warp_id / Parallel_Compute) * Parallel_Compute;
    // This warp's index within its row group:
    int warp_in_row_group = compute_warp_id % Parallel_Compute;
    // The number of rows skipped in the loop (distance between rows processed by the same warp group):
